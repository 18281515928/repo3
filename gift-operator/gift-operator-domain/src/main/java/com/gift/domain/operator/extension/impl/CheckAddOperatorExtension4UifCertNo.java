 ...
 */
@Slf4j
public class CheckAddOperatorExtension4UifCertNo implements IDomainParamCheckExtensionPoint {
    @Override
    public <T> DomainRequestParameterCheckDTO requestParameterCheckBeforeOpt(RequestDTO<T> requestDTO) {
        ...
        try {
            ...
            lambdaUpdateWrapper.eq(StringUtils.hasText(reqBody.getUifCertno()), CuOperatorInf::getUifCertno, reqBody.getUifCertno()).eq(StringUtils.hasText(reqBody.getUifCerttype()), CuOperatorInf::getUifCerttype, reqBody.getUifCerttype());
            //查询某个客户号下面得用户编号
...
//            DomainServiceUtil.transferUndeclaredThrowableException(e, new RespHeaderData(), ErrorCodeMsgEnum.FAIL.code(), e.getMessage());
        ...
        log.info("参数检查域能力-查询用户基本信息扩展点-根据操作员证件信息查询,responseDTO={}", responseDTO);
        return responseDTO;
    }
}
